...
class Runner:
    def __init__(self, args, conf_path):
        ...
        self.dataset = Dataset(args.input_dir, args.obj_name)
    ...
    def train(self):
        ...
        for iter_i in tqdm(range(self.iter_step, self.step2_maxiter)):
            ...
            if self.iter_step == self.step2_maxiter: 
                self.extract_mesh(resolution=self.args.mcube_resolution, threshold=0.0, point_gt=point_gt, iter_step=self.iter_step, logger=logger)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mcube_resolution', type=int, default=512)
